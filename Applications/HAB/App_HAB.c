...
//unsigned char uChar_app_hab_cutdown_timer;
//unsigned char uChar_app_hab_bitmask;
//const unsigned char uInt_app_hab_event_timer_cutdown=5;
...
void application_hab_cutdown_event_state_2(void){
	...
	if(struct_hab_cutdown_event_state.uInt_app_hab_timer >= HAB_CUTDOWN_EVENT_TIMER_COUNT_SECONDS){
		...
		//gpio_update(5, MOSFET_CNTL, 0); // Turn MOSFET OFF
		P5OUT &= ~MOSFET_CNTL;
